...
import CardImageRadio from "./form/card-image-radio";
...
const CardImageCheckboxCycle = (
  ...
  function onCheckedRadioImages(idUlid: string) {
    ...
    if (foundGalleryImages || newFoundGalleryImages) {
      foundGalleryImages && setSelectedImageProps(foundGalleryImages);
      newFoundGalleryImages && setSelectedImageProps(newFoundGalleryImages);

    } else {
      setSelectedImageProps(undefined);
      // setSelectedImageProps(prevImages => prevImages ? prevImages.filter((gallery) => gallery.ulid !== idUlid) : prevImages);
    }

    console.log('selectedImageProps', selectedImageProps);
  }
    ...
    <RadioGroup 
               ...
               onValueChange={(idUlid) => onCheckedRadioImages(idUlid)} 
               >
   <div className="grid grid-cols-7">
